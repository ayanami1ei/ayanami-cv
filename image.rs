...
// 可变行引用结构体
pub struct ImageRowMut<'a> {
    data: &'a mut [u8],
    width: usize,
    row_index: usize,
}

impl<'a> Index<usize> for ImageRow<'a> {
    type Output = u8;
...
impl<'a> Index<usize> for ImageRowMut<'a> {
    type Output = u8;
...
impl<'a> IndexMut<usize> for ImageRowMut<'a> {
    fn index_mut(&mut self, index: usize) -> &mut Self::Output {
        &mut self.data[index]
    }
}

impl<T, C: Channel> Image<T, C> 
where 
    T: Copy + Into<usize> + std::ops::Mul<Output = T> + From<u8> + PartialEq,
{
    ...
    // 获取行引用
    pub fn row(&self, row: usize) -> Option<ImageRow<'_>> {
        if row < self.height() {
    ...
    // 获取可变行引用
    pub fn row_mut(&mut self, row: usize) -> Option<ImageRowMut<'_>> {
        if row < self.height() {
            let start = row * self.width();
            let end = start + self.width();
    ...
    T: Copy + Into<usize> + std::ops::Mul<Output = T> + From<u8> + PartialEq,
{
    ...
    /// 转换为灰度图像（如果当前是彩色图像）
    pub fn to_grayscale(&self) -> Image<usize, crate::color_space::GrayChannel> {
        let mut gray_image = Image::<usize, crate::color_space::GrayChannel>::new(self.width(), self.height());
        ...
        for y in 0..self.height() {
            for x in 0..self.width() {
                if let Some(pixel) = self.pixel(x, y) {
                    ...
                    gray_image.set_pixel_by_coords(x, y, gray_value);
                }
            }
        }
