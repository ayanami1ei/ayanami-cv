...
// 可变行引用结构体
pub struct ImageHsvRowMut<'a> {
    data: &'a mut [u8],
    width: usize,
    row_index: usize,
}

impl<'a> Index<usize> for ImageHsvRow<'a> {
    type Output = u8;
...
impl<'a> Index<usize> for ImageHsvRowMut<'a> {
    type Output = u8;
...
impl<'a> IndexMut<usize> for ImageHsvRowMut<'a> {
    ...
    T: Copy + Into<usize> + std::ops::Mul<Output = T> + From<u8> + PartialEq,
{
    ...
    // 获取行引用
    pub fn row(&self, row: usize) -> Option<ImageHsvRow<'_>> {
        if row < self.height() {
    ...
    // 获取可变行引用
    pub fn row_mut(&mut self, row: usize) -> Option<ImageHsvRowMut<'_>> {
        if row < self.height() {
            let start = row * self.width();
            let end = start + self.width();
