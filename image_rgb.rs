...
pub struct ImageRgb<T> {
    ...
    height: T,
    data: Vec<u8>,
    _phantom: PhantomData<RgbChannel>,
}
...
// 可变行引用结构体
pub struct ImageRgbRowMut<'a> {
    data: &'a mut [u8],
    width: usize,
    row_index: usize,
}

impl<'a> Index<usize> for ImageRgbRow<'a> {
    type Output = u8;
...
impl<'a> Index<usize> for ImageRgbRowMut<'a> {
    type Output = u8;
...
impl<'a> IndexMut<usize> for ImageRgbRowMut<'a> {
    ...
    T: Copy + Into<usize> + std::ops::Mul<Output = T> + From<u8> + PartialEq,
{
    ...
    // 获取行引用
    pub fn row(&self, row: usize) -> Option<ImageRgbRow<'_>> {
        if row < self.height() {
    ...
    // 获取可变行引用
    pub fn row_mut(&mut self, row: usize) -> Option<ImageRgbRowMut<'_>> {
        if row < self.height() {
            let start = row * self.width();
            let end = start + self.width();
