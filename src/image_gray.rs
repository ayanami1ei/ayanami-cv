...
// 可变行引用结构体
pub struct ImageGrayRowMut<'a> {
    data: &'a mut [u8],
    width: usize,
    row_index: usize,
}

impl<'a> Index<usize> for ImageGrayRow<'a> {
    type Output = u8;
...
impl<'a> Index<usize> for ImageGrayRowMut<'a> {
    type Output = u8;
...
impl<'a> IndexMut<usize> for ImageGrayRowMut<'a> {
    ...
    T: Copy + Into<usize> + std::ops::Mul<Output = T> + From<u8> + PartialEq,
{
    ...
    // 获取行引用
    pub fn row(&self, row: usize) -> Option<ImageGrayRow<'_>> {
        if row < self.height() {
    ...
    // 获取可变行引用
    pub fn row_mut(&mut self, row: usize) -> Option<ImageGrayRowMut<'_>> {
        if row < self.height() {
            let start = row * self.width();
            let end = start + self.width();
            let width=self.width();
            Some(ImageGrayRowMut {
