...
// 可变行引用结构体
pub struct MatRgbRowMut<'a> {
    data: &'a mut [u8],
    width: usize,
    row_index: usize,
}

impl<'a> Index<usize> for MatRgbRow<'a> {
    type Output = u8;
...
impl<'a> Index<usize> for MatRgbRowMut<'a> {
    type Output = u8;
...
impl<'a> IndexMut<usize> for MatRgbRowMut<'a> {
    ...
    T: Copy + Into<usize> + std::ops::Mul<Output = T> + From<u8> + PartialEq,
{
    ...
    // 获取行引用
    pub fn row(&self, row: usize) -> Option<MatRgbRow<'_>> {
        if row < self.height() {
    ...
    // 获取可变行引用
    pub fn row_mut(&mut self, row: usize) -> Option<MatRgbRowMut<'_>> {
        if row < self.height() {
            let start = row * self.width();
            let end = start + self.width();
